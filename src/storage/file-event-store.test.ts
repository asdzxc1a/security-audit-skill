...
test("event stream rejects a domain schema downgrade from v3 back to v2", () => {
  const root = tempRoot();
...
test("schema-v2 assignment outcome rejects v3-only orchestrator_interrupted", () => {
  const root = tempRoot();
